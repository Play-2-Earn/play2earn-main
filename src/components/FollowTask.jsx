...
import { FaTwitter, FaLinkedin, FaTelegram, FaYoutube, FaUpload, FaInstagram, FaTiktok, FaWhatsapp } from 'react-icons/fa';
...
const FollowTask = () => {
  ...
  const updatePopupPosition = useCallback(() => {
    if (popupRef.current) {
      ...
      top = Math.max(scrollY + 20, Math.min(top, document.body.scrollHeight - popupHeight - 20));
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://localhost:5002/api/follow-task/api/follow-tasks');
      ...
      showNotification('Failed to fetch tasks. Please try again later.', 'error');
    }
  };
  ...
  const fetchUserData = async (userId) => {
    try {
      const response = await axios.get(`http://localhost:5002/api/follow-task/user/${userId}`);
      setTasks(prevTasks => {
  ...
  const handleScreenshotUpload = useCallback(async (platform, accountLink, file) => {
    ...
    try {
        const response = await axios.post('http://localhost:5002/api/follow-task/upload-screenshot', formData, {
            ...
            updatedTasks[platform] = updatedTasks[platform].map(task => {
              if (task.accountLink === accountLink) {
                return {
                  ...
                  screenshotUrl: URL.createObjectURL(file), // Create a local URL for the image
                  expanded: true
                };
              }
              return task;
  ...
  const showNotification = (message, type) => {
    if (screenWidth > 768) {  // Only show for desktop (assuming 768px as the breakpoint)
      setNotification({ message, type });
      setTimeout(() => setNotification(null), 5000);
    }
  };

  const handleShareAchievement = async (platform, accountLink, socialPlatform) => {
    showNotification(`Sharing ${platform} (${accountLink}) achievement on ${socialPlatform}...`, 'info');
    
    const websiteUrl = 'http://www.play2earn.ai/';
    const shareText = encodeURIComponent(`I just completed the ${platform} follow task on Play2Earn website and earned 10 points! Join now: ${websiteUrl}`);
    let shareUrl;
    ...
    switch (socialPlatform) {
      ...
      case 'whatsapp':
        ...
        break;
      case 'telegram':
        shareUrl = `https://t.me/share/url?url=${encodeURIComponent(websiteUrl)}&text=${shareText}`;
        break;
      default:
        shareUrl = null;
    }
    ...
    setTimeout(async () => {
      try {
        const response = await axios.post('http://localhost:5002/api/follow-task/share-achievement', {
          userId,
          platform,
          accountLink,
          socialPlatform
    ...
    return (
      <div className="share-options">
        ...
        <div className="share-buttons">
          <button onClick={() => handleShareAchievement(platform, accountLink, 'twitter')}><FaTwitter /> Twitter</button>
          <button onClick={() => handleShareAchievement(platform, accountLink, 'whatsapp')}><FaWhatsapp /> WhatsApp</button>
          <button onClick={() => handleShareAchievement(platform, accountLink, 'telegram')}><FaTelegram /> Telegram</button>
        </div>
      </div>
    );
  });
      ...
      <div className="follow-task-container">
        ...
        {Object.entries(tasks).map(([platform, platformTasks]) => (
         ...
         {notification && screenWidth > 768 && (
          <div className={`notification ${notification.type} desktop-notification`}>
