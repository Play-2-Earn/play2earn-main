...
const FollowTask = () => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await axios.get('http://localhost:5001/api/follow-task/api/follow-tasks');
  ...
  const fetchUserData = async (userId) => {
    try {
      const response = await axios.get(`http://localhost:5001/api/follow-task/user/${userId}`);
      setTasks(prevTasks => {
  ...
  const handleScreenshotUpload = useCallback(async (platform, accountLink, file) => {
    ...
    try {
      const response = await axios.post('http://localhost:5001/api/follow-task/upload-screenshot', formData, {
  ...
  const handleShareAchievement = async (platform, accountLink, socialPlatform) => {
    ...
    setTimeout(async () => {
      try {
        const response = await axios.post('http://localhost:5001/api/follow-task/share-achievement', {
          userId,
          platform,
          accountLink,
          socialPlatform
