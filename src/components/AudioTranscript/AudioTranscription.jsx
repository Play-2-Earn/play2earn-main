...
import { FaHeart, FaGem, FaArrowLeft, FaTimes, FaLock, FaUnlock, FaTrophy, FaTwitter, FaWhatsapp, FaTelegram } from 'react-icons/fa';
...
const AudioTranscription = () => {
  ...
  const handleSubmit = () => {
    ...
    const correctWords = currentTask.correctTranscription.toLowerCase().split(' ');
    ...
    if (accuracy >= 95) {
      pointsEarned = 3;
      setFeedbackMessage("Excellent job! Your transcription is highly accurate.");
    } else if (accuracy >= 85) {
      pointsEarned = 2;
      ...
      setLives(lives - 1);
      setFeedbackMessage("Keep practicing! Your transcription needs improvement.");
    }
    ...
    return tasks.map((task, index) => (
      <button
        ...
        className={`level-button ${
          index + 1 > currentLevel ? 'locked' : index + 1 <= completedLevels.length ? 'completed' : ''
        }`}
  ...
  return (
    <div className="audio-transcription-game">
      <div className={`app ${gameState !== 'initial' ? 'expanded' : 'initial'} ${isTransitioning ? 'transitioning' : ''}`}>
          ...
          <>
            ...
            <div className="point-lives">
              ...
              <div className="lives">
                Lives:{' '}
                {[...Array(3)].map((_, index) => (
                  <FaHeart key={index} className={index < lives ? 'heart' : 'heart empty'} />
                ))}
              </div>
            </div>
          </>
